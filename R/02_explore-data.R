...
df <- read_csv(file = "PLs_1946-2018.csv") %>% 
  select(ano, txtNomeAutor, txtSiglaPartido, txtSiglaUF, txtEmenta, situacao.descricao) %>% 
  mutate(Ano = ano,
...
df %>%
  ...
  labs(title = "Deputados Federais com 20 anos ou mais no cargo",
       x = "")
...
df %>% 
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Projetos de Lei por Deputados Federais", 
...
hoje_aguard_igual <- 
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Projetos de Lei com Situação igual a 'Aguardando'",
...
hoje_aguard_diff <- 
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Projetos de Lei com Situação diferente de 'Aguardando'",
       subtitle = "Projetos de Lei de Dep. Federais com 20 anos ou mais no cargo e que atualmente estão em exercício.", 
       x = "", y = "Quantidade de Projetos de Lei") 
